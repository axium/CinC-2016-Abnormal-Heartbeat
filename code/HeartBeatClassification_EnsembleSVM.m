...
Kernel = ['RBF' ; 'RBF'; 'RBF'];
L = [7.015, 7.015, 7.015];
Standardisation =[ 'on' ; 'on'; 'on'];

% [ValidationAccuracy, confusionMatrix, SVMModel_1, SVMModel_2, SVMModel_3] =...
%                    EnsembleSVM_kFold_VALID(DATA, TARGET, kFold, Kernel,  C, L, Standardisation)


%% TRAINING THE SELECTED MODEL ON ALL THE DATA.

[SVMModel_1, SVMModel_2, SVMModel_3] = ...
...
%   ENSEMBLE SVM PREDICTION ON TEST DATA
    ...
    %DATA = DATA_PercCepsL(end-TestDataSize+1 : end,   1:end-1)';
    %TARGET = DATA_PercCepsL(end-TestDataSize+1:end, end);

    Target_Predicted =  EnsembleSVM_PREDICT( DATA, SVMModel_1, SVMModel_2, SVMModel_3);
